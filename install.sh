...
#   curl -fsSL ... | bash -s -- --with-sandbox    # + bubblewrap isolation
#
# ============================================================================
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --with-sandbox Install bubblewrap sandbox (secure command execution)"
...
# Try to install Python via the OS package manager as a fallback
# when uv cannot provide it (e.g. permission-denied on $HOME/uv.toml,
# readonly filesystem, or network restrictions).
_install_python_fallback() {
    # Debian / Ubuntu
    if command -v apt-get >/dev/null 2>&1; then
        log_info "Trying apt-get fallback for Python $PYTHON_VERSION..."
        apt-get update -qq 2>/dev/null || true
        # Try direct install first (works on Debian 12+ and older Ubuntu LTS)
        if apt-get install -y -qq "python${PYTHON_VERSION}" "python${PYTHON_VERSION}-venv" "python${PYTHON_VERSION}-dev" 2>/dev/null; then
            PYTHON_PATH="$(command -v "python${PYTHON_VERSION}")"
            PYTHON_FOUND_VERSION="$("$PYTHON_PATH" --version 2>/dev/null)"
            return 0
        fi
        # Direct install failed — Ubuntu 24.04+ ships Python 3.12, try deadsnakes PPA
        log_info "Adding deadsnakes PPA..."
        apt-get install -y -qq software-properties-common >/dev/null 2>&1 || true
        add-apt-repository -y ppa:deadsnakes/ppa >/dev/null 2>&1 || true
        apt-get update -qq 2>/dev/null || true
        if apt-get install -y -qq "python${PYTHON_VERSION}" "python${PYTHON_VERSION}-venv" "python${PYTHON_VERSION}-dev" >/dev/null 2>&1; then
            PYTHON_PATH="$(command -v "python${PYTHON_VERSION}")"
            PYTHON_FOUND_VERSION="$("$PYTHON_PATH" --version 2>/dev/null)"
            return 0
        fi
    ...
    if command -v dnf >/dev/null 2>&1; then
        log_info "Trying dnf fallback for Python $PYTHON_VERSION..."
        if dnf install -y "python${PYTHON_VERSION}" "python${PYTHON_VERSION}-devel" >/dev/null 2>&1; then
            PYTHON_PATH="$(command -v "python${PYTHON_VERSION}")"
            PYTHON_FOUND_VERSION="$("$PYTHON_PATH" --version 2>/dev/null)"
            return 0
        fi
    ...
    if command -v yum >/dev/null 2>&1; then
        log_info "Trying yum fallback for Python $PYTHON_VERSION..."
        if yum install -y "python${PYTHON_VERSION}" >/dev/null 2>&1; then
            PYTHON_PATH="$(command -v "python${PYTHON_VERSION}")"
            PYTHON_FOUND_VERSION="$("$PYTHON_PATH" --version 2>/dev/null)"
            return 0
        fi
    ...
    if command -v apk >/dev/null 2>&1; then
        log_info "Trying apk fallback for Python 3.11..."
        if apk add --no-cache "python3~3.11" "python3-dev~3.11" >/dev/null 2>&1; then
    ...
    if command -v pacman >/dev/null 2>&1; then
        log_info "Trying pacman fallback for Python..."
        if pacman -S --noconfirm python >/dev/null 2>&1; then
...
check_python() {
    ...
    # Python not found — use uv to install it.
    # UV_NO_CONFIG=1 avoids $HOME/uv.toml permission issues on systems with
    # restricted /root (readonly rootfs, immutable home, cap_drop ALL, etc.).
    log_info "Python $PYTHON_VERSION not found, installing via uv..."
    _uv_install_log="$(mktemp /tmp/hermes-uv-python.XXXXXX.log)"
    if UV_NO_CONFIG=1 "$UV_CMD" python install "$PYTHON_VERSION" >"$_uv_install_log" 2>&1; then
        ...
        # If uv failed, fall back to the system package manager
        log_warn "uv python install failed (exit $uv_exit_code)"
        if _install_python_fallback; then
            log_success "Python installed via system package manager: $PYTHON_FOUND_VERSION"
        else
            log_error "Failed to install Python $PYTHON_VERSION"
            log_info "Install Python $PYTHON_VERSION manually, then re-run this script"
            log_info "OR re-run with: curl -fsSL .../install.sh | UV_NO_CONFIG=1 bash"
            exit 1
        fi
    fi
}
...
# Creates a hardened wrapper at /usr/local/bin/hermes-sandbox that restricts
# filesystem access, isolates the network, and creates ephemeral /root /home /tmp.
# File system layout: /usr, /bin, /lib (etc.) are read-only bind-mounted from
# host. /root, /home, /tmp are tmpfs (volatile). Network and PID namespace
# are unshared. The sandbox inherits no capabilities and dies with its parent.
install_sandbox() {
    if [ "$WITH_SANDBOX" != true ]; then
        return 0
    fi

    log_info "Installing bubblewrap sandbox..."
    ...
    _install_bwrap() {
        if command -v apt-get >/dev/null 2>&1; then
            ...
            pacman -S --noconfirm bubblewrap >/dev/null 2>&1
        else
            return 1
        fi
    }
    ...
    if ! command -v bwrap >/dev/null 2>&1; then
        if ! _install_bwrap; then
            log_warn "bubblewrap not available for this distribution"
            log_info "Install manually: https://github.com/containers/bubblewrap"
            return 0
        fi
    fi

    log_success "bubblewrap $(bwrap --version 2>/dev/null || echo 'installed')"

    # Create hardened sandbox wrapper
    local wrapper_dst=""
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        wrapper_dst="/usr/local/bin/hermes-sandbox"
    else
        mkdir -p "$HOME/.local/bin" 2>/dev/null || true
        wrapper_dst="$HOME/.local/bin/hermes-sandbox"
    fi

    cat > "$wrapper_dst" << 'BWRAP_EOF'
#!/bin/bash
# hermes-sandbox: Execute commands inside a bubblewrap sandbox
#
# Security properties:
#   • Root filesystem is READ-ONLY (bind-mounted from host)
#   • /root, /home, /tmp, /var/tmp are tmpfs (erased on exit)
#   • Network, PID, IPC, UTS namespaces are unshared
#   • No capabilities (empty cap set)
#   • Dies with parent process (no orphaned sandboxes)
#   • No setuid binaries accessible
#
# Usage: hermes-sandbox [command...]
# Example: hermes-sandbox python3 -c 'print("safe")'

if [ $# -eq 0 ]; then
    echo "Usage: hermes-sandbox [command...]"
    echo ""
    echo "Run commands in an isolated bubblewrap sandbox."
    echo "The filesystem is read-only; /root, /home, /tmp are ephemeral."
    echo "Network access is blocked (--unshare-net)."
    exit 1
fi
...
BWRAP_EOF
    ...
    log_success "Sandbox wrapper: $wrapper_dst"

    # Self-test: verify the sandbox actually runs
    if bwrap --ro-bind / / --tmpfs /tmp --dev /dev --proc /proc \
             --unshare-pid --die-with-parent \
             /bin/true 2>/dev/null; then
        log_success "Sandbox self-test passed"
    else
        log_warn "Sandbox self-test failed — your kernel may lack user namespaces"
        log_info "Enable with: sysctl kernel.unprivileged_userns_clone=1"
    fi
}
...
print_success() {
    ...
    if [ "$WITH_SANDBOX" = true ]; then
        echo -e "   ${GREEN}hermes-sandbox${NC}     Run commands in isolated sandbox"
        echo ""
    fi

    echo -e "   ${GREEN}🛡️  Hardening${NC}           UFW, fail2ban, AppArmor, kernel, Tirith"
    echo ""
...
# Hardening Functions
# ============================================================================
# Applies UFW, fail2ban, kernel hardening, unattended-upgrades, SSH hardening,
# and Hermes-specific security configuration.
#
# SAFETY: NEVER locks out SSH. Checks:
#   1. SSH key exists before disabling password auth
#   2. Adds UFW allow rule for SSH port BEFORE enabling UFW
#   3. Does NOT change SSH port when running via SSH
#   4. Creates rollback script at /tmp/hermes-hardening-rollback.sh
# ============================================================================

hardening_system() {
    # Only root can harden the system
    if [ "$(id -u)" != "0" ]; then
        log_warn "System hardening requires root. Skipping."
        log_info "Re-run with sudo to enable protection."
        return 0
    fi
    ...
    if [ -n "${SSH_CONNECTION:-}" ] || [ -n "${SSH_TTY:-}" ] || [ -n "${SSH_CLIENT:-}" ]; then
        IS_SSH=true
        log_info "Detected SSH session — will NOT change SSH port to prevent lockout"
    fi
    ...
    if [ -f "$AUTH_KEYS_FILE" ] && [ -s "$AUTH_KEYS_FILE" ]; then
        HAS_SSH_KEY=true
        log_success "SSH key found — safe to harden SSH"
    else
        log_warn "No SSH authorized_keys found! Will NOT disable password auth."
        log_warn "Add your SSH key first: ssh-copy-id $(whoami)@$(hostname -I | awk '{print $1}')"
    fi

    # ─── Create rollback script ───
    cat > /tmp/hermes-hardening-rollback.sh << 'ROLLBACK'
#!/bin/bash
# Rollback Hermes hardening — restores pre-hardening state
echo "Rolling back Hermes hardening..."
set -e

# Disable UFW
if command -v ufw &>/dev/null; then
    ufw disable 2>/dev/null && echo "[✓] UFW disabled" || echo "[!] UFW was not enabled"
fi

# Stop fail2ban
if systemctl is-active --quiet fail2ban 2>/dev/null; then
    systemctl stop fail2ban 2>/dev/null && echo "[✓] fail2ban stopped" || true
fi

# Restore SSH config from backup
if [ -f /etc/ssh/sshd_config.bak.hermes ]; then
    cp /etc/ssh/sshd_config.bak.hermes /etc/ssh/sshd_config
    systemctl restart ssh 2>/dev/null || systemctl restart sshd 2>/dev/null || true
    echo "[✓] SSH config restored from backup"
fi

# Remove AppArmor profile
if [ -f /etc/apparmor.d/usr.local.bin.hermes ]; then
    aa-disable /etc/apparmor.d/usr.local.bin.hermes 2>/dev/null || true
    rm -f /etc/apparmor.d/usr.local.bin.hermes
    echo "[✓] AppArmor profile removed"
fi

echo "Rollback complete."
ROLLBACK
    chmod +x /tmp/hermes-hardening-rollback.sh
    ...
    if [ -f /etc/ssh/sshd_config ]; then
        local DETECTED_PORT
        DETECTED_PORT=$(grep -E "^Port [0-9]+" /etc/ssh/sshd_config 2>/dev/null | awk '{print $2}')
    ...
    log_success "UFW: SSH port $SSH_PORT allowed"

    # If gateway will be used, open 80/443
    ufw allow 80/tcp comment "HTTP (Hermes gateway)"
    ufw allow 443/tcp comment "HTTPS (Hermes gateway)"

    ufw --force enable
    log_success "UFW enabled (default deny incoming, allow SSH:$SSH_PORT, HTTP, HTTPS)"
...
JAIL
    ...
    log_success "fail2ban enabled (SSH protection on port $SSH_PORT)"
    ...
    # Backup original
    if [ ! -f /etc/ssh/sshd_config.bak.hermes ]; then
        cp /etc/ssh/sshd_config /etc/ssh/sshd_config.bak.hermes
    fi

    # Only disable password auth if SSH key exists
    if [ "$HAS_SSH_KEY" = true ]; then
        sed -i 's/^#\?PasswordAuthentication.*/PasswordAuthentication no/' /etc/ssh/sshd_config
        log_success "SSH: Password authentication disabled (key-based only)"
    else
        log_warn "SSH: Password auth NOT disabled (no key found)"
    fi

    # Disable root login with password
    sed -i 's/^#\?PermitRootLogin.*/PermitRootLogin prohibit-password/' /etc/ssh/sshd_config

    # Only change port if NOT connected via SSH
    if [ "$IS_SSH" = false ]; then
        sed -i "s/^#\?Port .*/Port $SSH_PORT/" /etc/ssh/sshd_config
        log_info "SSH port: $SSH_PORT (not changed — already configured)"
    else
        log_info "SSH port: kept at $SSH_PORT (SSH session active)"
    fi
    ...
    if sshd -t 2>/dev/null; then
        ...
        cp /etc/ssh/sshd_config.bak.hermes /etc/ssh/sshd_config
        systemctl restart ssh 2>/dev/null || systemctl restart sshd 2>/dev/null || true
        log_error "SSH restored to original. Check /etc/ssh/sshd_config manually."
    fi
    ...
    cat > /etc/sysctl.d/99-hermes-hardening.conf << 'KERNEL'
# Network hardening
...
AUTO
    ...
    log_success "Automatic security updates enabled (daily, reboot at 03:00)"

    # ═══════════════════════════════════════════════════════════════
    # 6. AppArmor Profile (if available)
    # ═══════════════════════════════════════════════════════════════
    if command -v apparmor_parser &>/dev/null; then
        log_info "Creating AppArmor profile for Hermes..."

        cat > /etc/apparmor.d/usr.local.bin.hermes << 'AAPROFILE'
#include <tunables/global>

profile hermes /usr/local/bin/hermes flags=(attach_disconnected) {
    ...
    # Hermes directories
    owner /root/.hermes/** rw,
    owner /root/.hermes/ rw,
    owner /home/*/.hermes/** rw,
    owner /home/*/.hermes/ rw,

    # Hermes binary
    /usr/local/lib/hermes-agent/** mr,
    /usr/local/bin/hermes rix,
    ...
    deny /etc/shadow rwxlkm,
    deny /etc/sudoers rwxlkm,
    deny /root/.ssh/** rwxlkm,
    deny /home/*/.ssh/** rwxlkm,
    deny /home/*/.aws/** rwxlkm,
    deny /usr/bin/sudo x,
    deny /usr/bin/su x,
...
AAPROFILE
        ...
        apparmor_parser -r /etc/apparmor.d/usr.local.bin.hermes 2>/dev/null || true
        aa-enforce /etc/apparmor.d/usr.local.bin.hermes 2>/dev/null || true
        log_success "AppArmor profile created and enforced"
    else
        log_info "AppArmor not available on this system — skipping"
    fi

    log_success "System hardening complete"
    echo ""
    log_info "Rollback script: /tmp/hermes-hardening-rollback.sh"
    echo ""
}

# Apply Hermes-specific security configuration (tirith, tool restrictions, audit logging)
hardening_hermes_config() {

    local CONFIG_FILE="$HERMES_HOME/config.yaml"

    if [ ! -f "$CONFIG_FILE" ]; then
        log_warn "config.yaml not found at $CONFIG_FILE — skipping Hermes security config"
        return 0
    fi

    log_info "Applying Hermes security configuration..."

    # Backup
    cp "$CONFIG_FILE" "${CONFIG_FILE}.bak"

    # Check if tirith/security section already exists
    if grep -q "tirith_enabled" "$CONFIG_FILE" 2>/dev/null; then
        log_info "Security config already present — keeping existing"
        return 0
    fi

    # Append security configuration
...
# Disable tools that could be abused via prompt injection:
#   browser    — prevents drive-by attacks and data exfiltration
#   delegation — prevents subagent escape from sandbox
#   cronjob    — prevents persistence and scheduled attacks
#   image_gen  — prevents resource abuse
tools:
  disabled_toolsets:
    - "browser"
    - "delegation"
...
# Context Compression (prevents overflow attacks)
compression:
  enabled: true
  threshold: 0.40
  target_ratio: 0.15

# Session Checkpoints (rollback safety)
checkpoints:
  enabled: true
  max_snapshots: 25

# Audit Logging (track all tool calls and prompts)
logging:
  audit_tool_calls: true
  audit_prompts: true
HERMESCONFIG

    log_success "Hermes security config applied:"
    log_info "  ✓ Tirith prompt injection guard"
    log_info "  ✓ Disabled tools: browser, delegation, cronjob, image_gen"
    log_info "  ✓ Blocklist: pastebin, webhook.site, ngrok, localtunnel.me"
    log_info "  ✓ Context compression (threshold: 40%)"
    log_info "  ✓ Session checkpoints (max 25 snapshots)"
    log_info "  ✓ Audit logging (tool calls + prompts)"
    log_info "  Backup: ${CONFIG_FILE}.bak"
}
# ============================================================================
